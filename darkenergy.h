	...
	if(ndyn!=0)
	{
		...
		/*else																		//	Calculated from tabulated data. Skip it for this moment						
		{
         ...
         dynrho1=logrho[klo]-3.0*((1+wd[klo]-dwda[klo]*aw[klo])*dlna+dwda[klo]*(a-aw[klo]));
			...
			ah = a_ndemax[indhi]-a_ndemax[indlo];
			aa = (a_ndemax[indhi]-a)/ah;
			ab = (a-a_ndemax[indlo])/ah;

			if(a < a_ndemax[1]) 
				fint = omg_de[1];
			else if (a > a_ndemax[ndemax])
				fint = omg_de[ndemax];
			else 
				fint = omg_de[indlo]*aa + omg_de[indhi]*ab;


			// fint = fint * (1 - omegac - omegab)/omg_de[ndemax];
			return fint;
			//grho_de = grhom*fint*a*a*a*a;
			}
		}		
	else
		dynrho1=1.0;
	//printf("Now: %e",dynrho1);
	return dynrho1;
	}
